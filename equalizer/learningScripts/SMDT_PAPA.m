...
for barGammaNonLinIndex = 1:length(barGammaNonLin)
    ...
    for LIndex = 1:length(L)
        ...
        globalLength = maxRuns + N(NIndex) + L(LIndex) - 1;
        ...
        for index = 1:maxIt
            ...
            G1 = zeros(N(NIndex),adapFiltLength(N(NIndex)),globalLength);
            
            G2 = zeros(adapFiltLength(N(NIndex)) - N(NIndex),adapFiltLength(N(NIndex)),globalLength);
            ...
            w1 = zeros(N(NIndex),globalLength) + 1e-6;
            w2 = zeros(adapFiltLength(NIndex) - N(NIndex),globalLength) + 1e-6;
            ...
            for k = (adapFiltLength(NIndex) + max(delayVector2)):globalLength
                ...
                G(1:N(NIndex),1:N(NIndex),k) = diag(((1 - kappa*muLin(k))/N(NIndex)) + (kappa*muLin(k)*abs(w1(:,k))/norm(w1(:,k),1)));
                G(N(NIndex)+1:end,N(NIndex)+1:end,k) = diag(((1 - kappa*muNonLin(k))/(adapFiltLength(NIndex)-N(NIndex))) + (kappa*muNonLin(k)*abs(w2(:,k))/norm(w2(:,k),1)));
                
                if absoluteValueError > barGammaLin
                    w1(:,k+1) = w1(:,k) + muLin(k) * G(1:N(NIndex),1:N(NIndex),k)*xLin(:,k)*((xAP'* G(:,:,k)*xAP+gamma*eye(L(LIndex)+1))\eye(L(LIndex)+1))*conj(e(k))*u;
                    countLin(k,index) = 1;
                else
                    w1(:,k+1) = w1(:,k);
                end
                ...
                if absoluteValueError > barGammaNonLin(barGammaNonLinIndex)
                    w2(:,k+1) = w2(:,k) + muNonLin(k)*G(N(NIndex)+1:end,N(NIndex)+1:end,k)*xNonLin*((xAP'*G(:,:,k)*xAP+gamma*eye(L(LIndex)+1))\eye(L(LIndex)+1))*conj(e(k))*u;
