...
fileVector = {'resultsTest' 'resultsTest2'};
        ...
        for i = 5:size(e4,1)
            for j = 1:size(e4{i,m},1)
                ...
                for k = 1:size(e4{i,m},2)
                    ...
                    plot(xAux)
                    blindItAux = blindIt(:,k,i,m);
                    blindItAux2 = blindIt2(:,k,i,m);
                    if m == 1
                        updatesSup = mean(meanCount2{i,m}{j,k}(aux:4999))*100;
                        updatesBlind = mean(meanCount2{i,m}{j,k}(aux:4999))*100;
                        
                    else
                        meanBlindIt(l,m,i,j,k) = round(mean(blindItAux(blindItAux~=0)));
                        meanBlindIt2(l,m,i,j,k) = round(mean(blindItAux2(blindItAux2~=0)));
                        updatesSup = mean(meanCount2{i,m}{j,k}(aux:meanBlindIt(l,m,i,j,k)))*100;
                        updatesBlind = mean(meanCount2{i,m}{j,k}(meanBlindIt(l,m,i,j,k):4999))*100;
                    end
                    
                    hold on
                    updates = mean(meanCount2{i,m}{j,k}(aux:4999))*100;
