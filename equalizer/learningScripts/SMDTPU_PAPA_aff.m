...
for NIndex = 5:length(N)
    ...
    for barGammaNonLinIndex = 1:1%length(barGammaNonLin)
        barGammaNonLinIndex
         ...
         for delay = 1:length(delayVector)
            ...
            for index = 1:maxIt
                ...
                for k = (adapFiltLength(NIndex) + max(delayVector2)):globalLength
                    ...
                    p(k+1) = eta*p(k) + (1-eta)*((y1(k)-y2(k))^2);
                    
                    w(:,k+1) = 1*(lambdaUp(k)*w1(:,k+1)+ (1-lambdaUp(k))*w2(:,k+1));
                    
                    lambdaUp(k+1) = lambdaUp(k) + muLambda*e(k)*(y1(k) - y2(k))/(1e-8 + p(k+1));
...
save(['.' filesep 'results' filesep 'resultsTest2.mat'],'w4','e4','meanCountLin2','meanCountNonLin2');
