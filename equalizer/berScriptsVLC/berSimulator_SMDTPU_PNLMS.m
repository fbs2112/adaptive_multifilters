...
filterFile = 'results_SMDTPU_VLC_03';
...
signalPower = 1;
ber = zeros(length(SNR),size(e4,1),size(e4,2),size(w4{1,1},2));
...
for SNRIndex = 1:length(SNR)
    ...
    for NIndex = 1:size(e4,1)
        for modulationIndex = 1:size(e4,2)
            ...
            for barGammaNonLinIndex = 1:size(w4{NIndex,modulationIndex},2)
            equalizerFilter = [];
            berAux = zeros(monteCarloLoops,1);
            equalizerFilter(:,1) = squeeze(w4{NIndex,modulationIndex}{1,barGammaNonLinIndex}(:,end));
            ...
            ber(SNRIndex,NIndex,modulationIndex,barGammaNonLinIndex) = mean(berAux);
            end
        end
    end
