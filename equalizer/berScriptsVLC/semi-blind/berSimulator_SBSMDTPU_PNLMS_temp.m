...
filterFile = 'resultsSBSMDTPU_VLC_01';
...
signalPower = 1;
ber = zeros(length(SNR),size(e4,1),size(e4,3),size(e4,2),size(w4{1,1},2));
        ...
        for modulationIndex = 1:size(e4,3)
            ...
            for etaIndex = 1:1%size(e4,2)
                ...
                for barGammaNonLinIndex = 1:size(w4{1,1},2)
                    ...
                    equalizerFilter(:,1) = squeeze(w4{NIndex,etaIndex,modulationIndex}{1,barGammaNonLinIndex}(:,end));
                    ...
                    ber(SNRIndex,NIndex,modulationIndex,etaIndex,barGammaNonLinIndex) = mean(berAux);
                end
            end
        end
    end
